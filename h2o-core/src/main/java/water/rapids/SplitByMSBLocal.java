    ...
    for (int r=0; r<chk[0]._len; r++) {    // tight, branch free and cache efficient (surprisingly)
     ...
     //long thisx = 0;
      ...
      if (!chk[0].isNA(r)) {
        ...
        if (_id_maps[0]!=null) { // dealing with enum columns
          thisx = BigInteger.valueOf(_isLeft?(int)chk[0].at8(r) + 1:_id_maps[0][(int)chk[0].at8(r)] + 1);
      ...
      for (int c=1; c<chk.length; c++) {  // TO DO: left align subsequent
        ...
        if (_id_maps[c] != null) {
          thisx = BigInteger.valueOf(_isLeft?(int)chk[c].at8(r) + 1:_id_maps[c][(int)chk[c].at8(r)] + 1);
