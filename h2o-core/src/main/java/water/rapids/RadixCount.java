...
class RadixCount extends MRTask<RadixCount> {
  ...
  @Override public void map( Chunk chk ) {
    ...
    if (chk.vec().isCategorical()) {
      ...
      if (chk.vec().naCnt() == 0) {
        for (int r=0; r<chk._len; r++) { // mapping of enum level to MSB not intuitive, multiple enums map to same MSB
          int ctrVal = _isLeft?BigInteger.valueOf((int)chk.at8(r)+1).shiftRight(_shift).intValue()
                  :BigInteger.valueOf(_id_maps[0][(int)chk.at8(r)]+1).shiftRight(_shift).intValue();
          tmp[ctrVal]++;
